...
namespace JA.Numerics.Simulation
{
    ...
    public static partial class Ode
    {
        ...
        /// <summary>Implementation of Runge-Kutta algorithm with per-point accuracy control from
        /// Dormand and Prince article.
        /// (J.R.Dormand, P.J.Prince, A family of embedded Runge-Kutta formula)</summary>
        ...
        public static IEnumerable<SolPoint> RK547M(double t0, Vector x0, Func<double, Vector, Vector> f, Options opts)
        {
            ...
            {
                Vector.Copy(x1, ref prevX);
                double e = 0.0d; // error factor, should be < 1
                //int ii = 0;
                do
                {
                    prevDt = dt;
                    Vector.Copy(prevX, ref x1);
                    // Compute internal method variables.
                    k[0] = dt * f(t, x1);
                    for (int i = 1; i < s; i++)
                    {
                        Vector.Copy(x1, ref x2[i - 1]);
                        for (int j = 0; j < i; j++)
                        {
        ...
        public static IEnumerable<SolPoint> RK547M(double tstart, double tfinal, Vector x0, Func<double, Vector, Vector> f, Options opts)
        {
            ...
            return RK547M(tstart, x0, f, opts).SolveFromTo(tstart, tfinal);
        }
