...
namespace JA.Numerics
{
    ...
    public class SparseMatrix
    {
        ...
        public SparseMatrix times(SparseMatrix B)
        {
            ...
            if (B.m != n)
            {
                throw new ArgumentException("Sparse natrix inner dimensions must agree.");
            }
        ...
        public SparseMatrix Transpose()
        {
            ...
            for (int i = 0; i < RowDimension; i++)
                for (int j = 0; j < this[i]._count; j++)
                    At[this[i]._indexes[j], i] = this[i]._items[j];
        ...
        public SparseVector this[int i]
        {
            get
            {
                return new SparseVector(n, items[i], indices[i]);
            }
            set
            {
                indices[i] = value._indexes;
                items[i] = value._items;
        ...
        private void CheckMatrixDimensions(Matrix B)
        {
            ...
            {
                throw new ArgumentException("Sparse matrix dimensions must agree.");
            }
        }
    }
}